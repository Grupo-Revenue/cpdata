      ...
      <div className="flex items-center justify-between border-b-2 border-gray-800 pb-4 mb-6">
        <div className="flex items-center gap-4">
          {brandConfig?.logo_url && (
            <img 
              src={brandConfig.logo_url.startsWith('data:') 
                ? brandConfig.logo_url 
                : brandConfig.logo_url.startsWith('http') 
                  ? brandConfig.logo_url
                  : `${window.location.origin}${brandConfig.logo_url}`
              } 
              alt={`${brandConfig.nombre_empresa} Logo`} 
              className="max-h-20 w-auto object-contain max-w-[200px]"
              style={{ aspectRatio: 'auto', objectFit: 'contain' }}
              onError={(e) => {
                console.error('❌ Error loading logo:', {
                  originalUrl: brandConfig.logo_url,
                  finalUrl: e.currentTarget.src,
        ...
        <div className="grid grid-cols-2 gap-8">
          ...
          <div>
            ...
            <div className="text-sm space-y-1">
              ...
              <div><span className="font-semibold">Fecha:</span> {negocio.evento.fechaEvento ? `${formatearFechaSinZonaHoraria(negocio.evento.fechaEvento)}${negocio.evento.fechaEventoFin ? ` - ${formatearFechaSinZonaHoraria(negocio.evento.fechaEventoFin)}` : ''}` : 'Por definir'}</div>
