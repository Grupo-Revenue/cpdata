...
BEGIN
    ...
    IF estado_anterior IS DISTINCT FROM estado_nuevo THEN
        ...
        -- Solo si net.http_post está disponible (verificar si la extensión está habilitada)
        BEGIN
            PERFORM
                net.http_post(
        ...
        EXCEPTION
            WHEN OTHERS THEN
                -- Si falla la llamada HTTP, solo loguear el error
                UPDATE hubspot_sync_log 
                SET status = 'failed',
                    error_message = SQLERRM,
                    processed_at = now()
                WHERE negocio_id = negocio_id_var 
                AND operation_type = 'estado_change' 
                AND status = 'pending'
